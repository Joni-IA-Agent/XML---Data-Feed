...
def main():
    ...
    # Load branded image manifest (generated by generate_branded_images.py)
    image_manifest: dict = {}
    if MANIFEST_PATH.exists():
        with open(MANIFEST_PATH, encoding="utf-8") as f:
            image_manifest = json.load(f)
        print(f"📸 Loaded image manifest: {len(image_manifest)} branded images")
    else:
        print("⚠  No image manifest found — falling back to scraped/Unsplash images")
        print("   Run: python tools/generate_branded_images.py to generate branded images")
...
-->\n''')
    ...
    for product in products:
        pid    = product["id"]
        # Prefer branded image from manifest; fall back to scraped/Unsplash
